...
export class MonthsSelect extends Component {
    ...
    return (
      <div className="months-wrapper">
        <div className="months-controls">
          <AutoTextbox
            label="From"
            value={this.months[from]}
            options={this.months}
            onChange={value => this.startAt(value)}
          />
          <AutoTextbox
            label="To"
            value={this.months[to]}
            options={this.months}
        ...
        <div className="months-view">
          ...
          <div className="months">{this.listMonths()}</div>
        </div>
      </div>
    );
  }
  ...
  listMonths() {
    const { from, to, except } = this.props;
  ...
  select(i) {
    ...
    if (i > from && i < to) {
      ...
      if (index === -1) {
        ...
        this.makeChange({
          except: [...except.slice(0, index), ...except.slice(index + 1)]
        });
      }
    }
  }
